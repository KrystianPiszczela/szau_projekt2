...
for k=kp:P
    y_mod_ucz(k)= w_ucz(1)*u_ucz(k-3) + w_ucz(2)*u_ucz(k-4) + w_ucz(3)*y_mod_ucz(k-1) + w_ucz(4)*y_mod_ucz(k-2);
    E_ucz = E_ucz + (y_mod_ucz(k)-y_ucz(k))^2; 
    y_mod_wer(k)= w_wer(1)*u_wer(k-3) + w_wer(2)*u_wer(k-4) + w_wer(3)*y_mod_wer(k-1) + w_wer(4)*y_mod_wer(k-2);
